...
async def on_ready():
    ...
    # set presence
    try:
        await bot.change_presence(activity=discord.Game(name="â›„â€¢ Stumble Hour"))
    except Exception:
        pass
...
async def lock(ctx):
    try:
        ...
        await ctx.send("âŒ I don't have permission to change channel permissions. Ensure my role is high enough and I have Manage Roles.", delete_after=10)
    except Exception as e:
        await ctx.send(f"âŒ An error occurred while locking the channel: {e}", delete_after=10)

@lock.error
async def lock_error(ctx, error):
    if isinstance(error, commands.MissingPermissions):
        await ctx.send("âŒ You need the Manage Channels permission to lock channels.", delete_after=8)
    else:
        await ctx.send(f"âŒ Error: {error}", delete_after=8)
...
async def unlock(ctx):
    try:
        ...
        await ctx.send(f"âŒ An error occurred while unlocking the channel: {e}", delete_after=10)

@unlock.error
async def unlock_error(ctx, error):
    if isinstance(error, commands.MissingPermissions):
        await ctx.send("âŒ You need the Manage Channels permission to unlock channels.", delete_after=8)
    else:
        await ctx.send(f"âŒ Error: {error}", delete_after=8)
...
@tree.command(name="game", description="Create a game download panel with version info and download links")
@app_commands.describe(
    sg_build_vers="Stumble Guys build version",
    stumble_hour_vers="Stumble Hour version",
    phone_link="Download link for phone (Android)",
    pc_link="Download link for PC (Game Folder)",
    loader_link="Download link for Loader DLL"
)
async def game_command(
    interaction: discord.Interaction,
    sg_build_vers: str,
    stumble_hour_vers: str,
    phone_link: str,
    pc_link: str,
    loader_link: str
):
    ...
    timestamp_relative = discord.utils.format_dt(now, style='R')
    
    embed = discord.Embed(color=discord.Color.yellow())
    
    embed.add_field(
        name="",
        value=f"⏱️ Last Updated: {timestamp_relative}\n🔹 Stumble guys build version: {sg_build_vers}\n🔸 Stumble Hour version: {stumble_hour_vers}",
        inline=False
    )
    ...
    view.add_item(phone_button)
    
    pc_button = discord.ui.Button(
        label="PC download",
        style=discord.ButtonStyle.link,
        url=pc_link
    )
    view.add_item(pc_button)
...
async def warn(ctx, member: discord.Member = None, *, reason: str = "No reason provided"):
    ...
    try:
        ...
        try:
            # don't DM the user on warn to avoid DM errors; keep functionality minimal as requested
            pass
        except Exception:
            pass
    except Exception as e:
...
class DeleteWarningModal(discord.ui.Modal):
    def __init__(self, guild_id: int, target_user_id: int, message_id: int, channel_id: int):
        super().__init__(title="Delete Warning")
        self.guild_id = str(guild_id)
        self.target_user_id = str(target_user_id)
        self.orig_message_id = message_id
        self.orig_channel_id = channel_id
        # Simplified modal: single input labeled "Warning number" per request
        self.warn_number = discord.ui.TextInput(label="Warning number", style=discord.TextStyle.short, required=True, max_length=10)
        self.add_item(self.warn_number)

    async def on_submit(self, interaction: discord.Interaction):
        perms = interaction.user.guild_permissions
        if not (perms.moderate_members or perms.manage_guild or perms.manage_roles or perms.kick_members):
            await interaction.response.send_message("You don't have permission to delete warnings.", ephemeral=True)
            return

        try:
            idx = int(self.warn_number.value.strip())
        except Exception:
            await interaction.response.send_message("Invalid number provided.", ephemeral=True)
            return

        guild_warnings = warnings_db.get(self.guild_id, {})
        user_warnings = guild_warnings.get(self.target_user_id, [])
        if not user_warnings:
            await interaction.response.send_message("No warnings found for that user.", ephemeral=True)
            return
        if idx < 1 or idx > len(user_warnings):
            await interaction.response.send_message(f"Invalid warn number. Provide a number between 1 and {len(user_warnings)}", ephemeral=True)
            return

        removed = user_warnings.pop(idx - 1)
        if len(user_warnings) == 0:
            guild_warnings.pop(self.target_user_id, None)
        warnings_db[self.guild_id] = guild_warnings
        save_json_file(WARNINGS_PATH, warnings_db)

        # attempt to update the original message embed with the new warnings list
        try:
            ch = bot.get_channel(self.orig_channel_id) or await bot.fetch_channel(self.orig_channel_id)
            msg = await ch.fetch_message(self.orig_message_id)
            updated = build_warnings_embed(ch.guild, int(self.target_user_id))
            await msg.edit(embed=updated)
        except Exception:
            pass

        await interaction.response.send_message(f"Deleted warning #{idx}.", ephemeral=True)
...
class DeleteWarningView(discord.ui.View):
    ...
    @discord.ui.button(label="Delete Warning", style=discord.ButtonStyle.danger, custom_id="delete_warn_button")
    async def delete_warn_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        perms = interaction.user.guild_permissions
        if not (perms.moderate_members or perms.manage_guild or perms.manage_roles or perms.kick_members):
            await interaction.response.send_message("You don't have permission to delete warnings.", ephemeral=True)
            return
        modal = DeleteWarningModal(self.guild_id, self.target_user_id, self.message_id, self.channel_id)
        await interaction.response.send_modal(modal)

def build_warnings_embed(guild: discord.Guild, member_id: int):
    guild_id = str(guild.id)
    user_id = str(member_id)
    embed = discord.Embed(title=f"Warnings for {member_id}", color=0x3498DB, timestamp=discord.utils.utcnow())
    member = guild.get_member(member_id)
    if member:
        embed.title = f"Warnings for {member}"
        embed.set_thumbnail(url=member.display_avatar.url)
...
async def warnings(ctx, member: discord.Member = None):
    if member is None:
        ...
        return
    embed = build_warnings_embed(ctx.guild, member.id)
    # send with a DeleteWarning button (which opens a modal to choose the warn number)
    sent = await ctx.send(embed=embed, view=DeleteWarningView(ctx.guild.id, member.id, message_id=0, channel_id=ctx.channel.id))
    # Edit the message to attach a view with the correct message id so modal can update original embed
    view = DeleteWarningView(ctx.guild.id, member.id, sent.id, ctx.channel.id)
    try:
...
# Command shortcut to remove a warn
...
# ---------------- Moderation Commands (mute/ban) - fixes for aware datetimes and no DM for ban ----------------
...
async def mute(ctx, member: discord.Member = None, duration: str = "60s", *, reason: str = "No reason provided"):
    ...
    try:
        ...
        await ctx.send(embed=embed)
        try:
            # Don't attempt to DM if you want to avoid DM problems; keep silent on DM errors
            pass
        except Exception:
            pass
    except discord.Forbidden:
