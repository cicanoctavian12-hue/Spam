# full main.py with the requested change: application form now asks 5 questions (no username field)
import os
import re
import json
import asyncio
...
# ---------------- Application Modal (UPDATED: 5 questions) ----------------
class ApplicationModal(discord.ui.Modal, title="Content Creator Application"):
    ...
    # Updated fields exactly as requested (5 questions). Each max_length 500.
    q1_platform_play = discord.ui.TextInput(
        label="1. On which Platform do you play?",
        style=discord.TextStyle.short,
        max_length=500,
        required=True
    )
    q2_platform_post = discord.ui.TextInput(
        label="2. On which Platform do you Post Videos?",
        style=discord.TextStyle.short,
        max_length=500,
        required=True
    )
    q3_content_type = discord.ui.TextInput(
        label="3. What content do you do?",
        style=discord.TextStyle.paragraph,
        max_length=500,
        required=True
    )
    q4_age = discord.ui.TextInput(
        label="4. What is your age?",
        style=discord.TextStyle.short,
        max_length=500,
        required=True
    )
    q5_why = discord.ui.TextInput(
        label="5. Why do you want to be an Content Creator for Stumble Hour?",
        style=discord.TextStyle.paragraph,
        max_length=500,
        required=True
    )
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        answers = {
            "1": self.q1_platform_play.value,
            "2": self.q2_platform_post.value,
            "3": self.q3_content_type.value,
            "4": self.q4_age.value,
            "5": self.q5_why.value
        }

        # Build purple embed with the 5 Q/A entries
        embed = discord.Embed(title="Content Creator Application", color=0x7B3FBF)
        q_map = {
            1: "On which Platform do you play?",
            2: "On which Platform do you Post Videos?",
            3: "What content do you do?",
            4: "What is your age?",
            5: "Why do you want to be an Content Creator for Stumble Hour?"
        }
        parts = []
        for i in range(1, 6):
        ...
        # persist mapping (store applicant id and answers)
...
class ApplyButtonView(discord.ui.View):
    ...
    # persistent callback for apply button (custom_id stable)
    @discord.ui.button(label="Apply (persistent callback)", style=discord.ButtonStyle.primary, custom_id=APPLY_BUTTON_CUSTOM_ID, emoji=CREATOR_ICON)
...
# ---------------- /cc_apply slash command (with required 'button' option) ----------------
...
@app_commands.describe(
    ...
    button="Label for the panel button (required) - this will be the text shown on the blue button"
)
async def cc_apply(interaction: discord.Interaction, color: str, channel: discord.TextChannel, button: str):
    # Permission: allow only users with Manage Guild (server staff) to create the panel
    ...
    # validate button label length (Discord button labels up to 80 chars; keep it safe)
    button_label = button.strip()
    if not button_label:
        await interaction.response.send_message("Button label cannot be empty.", ephemeral=True)
        return
    if len(button_label) > 80:
        await interaction.response.send_message("Button label is too long (max 80 characters).", ephemeral=True)
        return
    ...
    # Create a View for sending with the user-specified label. Use same persistent custom_id so callback is matched.
    view = discord.ui.View(timeout=None)
    # Blue button = primary style, persistent custom_id
    try:
        view.add_item(discord.ui.Button(label=button_label, style=discord.ButtonStyle.primary, custom_id=APPLY_BUTTON_CUSTOM_ID, emoji=CREATOR_ICON))
    except Exception:
        # fallback to label without emoji if adding emoji fails
        view = discord.ui.View(timeout=None)
        view.add_item(discord.ui.Button(label=button_label, style=discord.ButtonStyle.primary, custom_id=APPLY_BUTTON_CUSTOM_ID))
