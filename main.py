...
class GameInviteView(discord.ui.View):
    def __init__(self, challenger, opponent, game_type):
    ...
    async def accept_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if self.game_type == "ttt":
            ...
            embed = discord.Embed(
                title="🎮 Tic-Tac-Toe",
                description=f"{self.challenger.mention} (X) vs {self.opponent.mention} (O)\n\n{self.challenger.mention}'s turn",
                color=discord.Color.blue()
            )
            await interaction.followup.send(embed=embed, view=view)
        elif self.game_type == "rps":
            view = RPSView(self.challenger, self.opponent)
            embed = discord.Embed(
...
async def tictactoe(ctx, opponent: discord.Member = None):
    ...
    if opponent:
        embed = discord.Embed(
            ...
            description=f"{ctx.author.mention} invited {opponent.mention} to a Tic-Tac-Toe game!",
            color=discord.Color.blue()
        )
        view = GameInviteView(ctx.author, opponent, "ttt")
        await ctx.send(embed=embed, view=view)
    else:
...
async def rockpaperscissors(ctx, opponent: discord.Member = None):
    ...
    if opponent:
        embed = discord.Embed(
            ...
            description=f"{ctx.author.mention} invited {opponent.mention} to a Rock Paper Scissors game!",
            color=discord.Color.blue()
        )
        view = GameInviteView(ctx.author, opponent, "rps")
        await ctx.send(embed=embed, view=view)
    else:
...
async def trivia(ctx):
    ...
    for round_num in range(1, rounds + 1):
        ...
        try:
            ...
            timeout_embed = discord.Embed(
                title="⏰ Time's Up!",
                description=f"No one answered! The answer was **{question['a']}**",
                color=discord.Color.orange()
            )
            await ctx.send(embed=timeout_embed)
...
async def math_game(ctx):
    ...
    try:
        ...
        try:
            ...
            await ctx.send(f"❌ Please enter a number!")
    except asyncio.TimeoutError:
...
async def typerace(ctx):
    ...
    spoiler_text = ''.join([f'||{char}||' for char in text])
    
    embed = discord.Embed(
        title="⌨️ Typing Race - Multiplayer!",
        description=f"Type the text below as fast as you can!\nClick to reveal each letter - First to type it correctly wins! (60 seconds)\n\n{spoiler_text}",
        color=discord.Color.green()
    )
    ...
    try:
        ...
        if msg.content.strip() == text:
            ...
            await ctx.send(f"❌ Not quite right! Keep trying!")
    except asyncio.TimeoutError:
