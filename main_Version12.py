...
# ---------------- /game slash command (table-like embed + URL buttons) ----------------
@tree.command(name="game", description="Create a download/info embed with optional link buttons.")
@app_commands.describe(
...
async def game_command(
    ...
    await interaction.response.send_message(
        "Interactive setup: I will ask for each section here. Reply to each prompt with the text to show in that section.",
        ephemeral=False
    )
    ...
    try:
        # Section 1 (required)
        await interaction.followup.send("What text do you want for SECTION 1? (This is required)", ephemeral=False)
        msg1 = await bot.wait_for("message", timeout=300.0, check=check)
        section1 = msg1.content.strip()

        # Last Updated (optional)
        await interaction.followup.send("Provide 'Last Updated' text (or send '-' to skip). Example: 'October 27, 2025'", ephemeral=False)
        try:
            lu_msg = await bot.wait_for("message", timeout=180.0, check=check)
            last_updated = lu_msg.content.strip()
        ...
        section2 = None
        if add_section2:
            await interaction.followup.send("What text do you want for SECTION 2?", ephemeral=False)
            try:
                m2 = await bot.wait_for("message", timeout=300.0, check=check)
                section2 = m2.content.strip()
            except asyncio.TimeoutError:
                section2 = None

        # Optional link sections
        phone_text = None
        if phone_link:
            await interaction.followup.send("You provided a phone link. What text should appear for the PHONE section? (this text appears above the button)", ephemeral=False)
            try:
                mp = await bot.wait_for("message", timeout=300.0, check=check)
                phone_text = mp.content.strip()
            except asyncio.TimeoutError:
                phone_text = None
        ...
        if loader_link:
            ...
            try:
                ...
                loader_text = ml.content.strip()
            except asyncio.TimeoutError:
                loader_text = None

    except asyncio.TimeoutError:
        await interaction.followup.send("Timed out waiting for responses. Run the command again.", ephemeral=True)
        return

    # Build embed: table-like using inline fields and multi-line fields
    EMBED_COLOR = 0xFFD54F  # warm yellow-ish
    title_text = "🎮  - StumblePriv Official Downloads"
    ...
    # Convert section1 into lines and render as table-like rows
    # If user provided multiple lines separated by newline, treat each line as a separate row.
    section1_lines = [ln.strip() for ln in section1.splitlines() if ln.strip()] if section1 else []
    # if single-line, keep as one field
    if len(section1_lines) == 1:
        embed.add_field(name="\u200b", value=section1_lines[0], inline=False)
    else:
        # Create two-column rows: label / value pairs when possible, otherwise single-line entries
        for line in section1_lines:
            # if line contains ":" treat as label:value
            if ":" in line:
                left, right = line.split(":", 1)
                # small formatted left (bold) and right as code block-like inline (using backticks)
                embed.add_field(name=left.strip(), value=right.strip(), inline=True)
            else:
    ...
    # Warning block (use a full-width field similar to the screenshot)
    warning_block = ("⚠️ **PC - Make sure to replace the old DLL with the new one we provided.**\n"
    ...
    embed.add_field(name="\u200b", value=warning_block, inline=False)

    # Buttons and link sections: create view with URL buttons (persist as links)
    view = discord.ui.View(timeout=None)

    # For visual grouping, create a field per download entry and add a corresponding URL button.
    if phone_text:
        embed.add_field(name="📱 - Mobile Download", value=phone_text or "\u200b", inline=False)
        view.add_item(discord.ui.Button(label="Download here (APK)", url=phone_link))

    if pc_text:
        embed.add_field(name="🖥️ - PC Download", value=pc_text or "\u200b", inline=False)
        view.add_item(discord.ui.Button(label="Download here (Game Folder)", url=pc_link))

    if loader_text:
        embed.add_field(name="🧩 - PC Download", value=loader_text or "\u200b", inline=False)
        view.add_item(discord.ui.Button(label="Download here (DLL)", url=loader_link))

    # Thumbnail: try to use guild icon (or leave blank)
    try:
        if interaction.guild and interaction.guild.icon:
            embed.set_thumbnail(url=interaction.guild.icon.url)
    except Exception:
