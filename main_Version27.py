...
# Replace these IDs with your server emoji IDs if different
...
class ApplyButtonView(discord.ui.View):
    ...
    # persistent callback for apply button
...
# ---------------- /cc_apply slash command (button label required) ----------------
...
async def cc_apply(interaction: discord.Interaction, color: str, channel: discord.TextChannel, button: str):
    ...
    panel_text = msg.content or "\u200b"
    embed = discord.Embed(description=panel_text, color=embed_color)

    # create an instance of ApplyButtonView and set the button label to the user-provided label
    view = ApplyButtonView()
    for child in view.children:
        if isinstance(child, discord.ui.Button) and getattr(child, "custom_id", None) == APPLY_BUTTON_CUSTOM_ID:
            try:
