...
async def on_ready():
    ...
    for giveaway_id in list(active_giveaways.keys()):
...
async def help_command(ctx):
    ...
    if is_admin:
        embed.add_field(
            name="🛡️ Moderation",
            value=(
                "`-ban <member> [reason]` - Ban member (Admin)\n"
        ...
        embed.add_field(
            ...
            value=(
                ...
                "`-add <member> <amount>` - Add coins (Admin)\n"
                "`-remove <member> <amount>` - Remove coins (Admin)"
            ),
            inline=False
        )
...
class DropButton(discord.ui.Button):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        min_reward, max_reward = rarity_rewards[self.rarity]
        
        weights = []
        for i in range(min_reward, max_reward + 1):
            weight = max_reward - i + 1
            weights.append(weight)
        
        reward = random.choices(range(min_reward, max_reward + 1), weights=weights, k=1)[0]
...
CHALLENGE_LIST = [
    "Send 5 messages in this server",
    "Claim your daily reward",
    "Use the -help command",
    "React to 3 messages with emojis",
    "Check your balance",
    "Send a message with an emoji",
    "Check the server leaderboard",
    "Use the -ping command",
    "Send 3 messages in this server",
    "Check someone else's balance",
    "Use any minigame command",
    "Send a GIF in chat",
    "Mention another user in a message",
    "Send 10 messages in this server",
    "Check the shop",
    "Use the -8ball command",
    "Send a message longer than 50 characters",
    "React to a message",
    "Send 2 messages in this server",
    "Use the -wyr command"
]
...
async def challenge_cmd(ctx):
    ...
    challenge_task = random.choice(CHALLENGE_LIST)
    ...
    embed = discord.Embed(
        title="🎯 New Challenge!",
        description=f"**Challenge:** {challenge_task}\n\n**Reward:** 30 {CURRENCY_EMOJI}",
        color=discord.Color.green()
    )
    embed.set_footer(text="Use -completechallenge when you're done!")
    await ctx.send(embed=embed)

@bot.command(name='completechallenge')
...
class GiveawayButton(discord.ui.Button):
    ...
    async def callback(self, interaction: discord.Interaction):
        if self.giveaway_id not in active_giveaways:
            await interaction.response.send_message("❌ This giveaway is no longer active or has ended!", ephemeral=True)
            return
        ...
        try:
            self.label = f"Participate ({len(participants)}/♾️)"
...
async def giveaway_slash(
    ...
    result_embed = discord.Embed(
        ...
        description=f"**{title}**\n\n🎁 **Prize:** {prize}\n👥 **Participants:** {len(participants)}\n\n{'🏆 Winner' if len(winners_list) == 1 else '🏆 Winners'}:",
        color=discord.Color.green()
    )
    
    for i, winner in enumerate(winners_list, 1):
        result_embed.add_field(name=f"{'🥇' if i == 1 else '🥈' if i == 2 else '🥉' if i == 3 else f'{i}.'} Winner", value=winner.mention, inline=False)
