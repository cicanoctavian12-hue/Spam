...
# PartialEmoji placeholders (bot must have access or emojis will render as text)
...
# Staff channel for applications (from your request)
STAFF_CHANNEL_ID = 1439265373697605683
...
class ApplicationModal(discord.ui.Modal, title="Content Creator Application"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        # create purple embed with Q/A structure
...
class ApplyButtonView(discord.ui.View):
    ...
    # Updated label to exactly "Apply here" and emoji set to the CreatorIcon partial emoji.
    @discord.ui.button(label="Apply here", style=discord.ButtonStyle.primary, custom_id=APPLY_BUTTON_CUSTOM_ID, emoji=CREATOR_ICON)
...
# ---------------- on_ready: register persistent views & presence ----------------
@bot.event
async def on_ready():
    ...
    # Register persistent views so buttons remain clickable after restarts
    try:
        # Register class instances here; also register below at import-time for extra safety
...
# IMPORTANT: register persistent views at import time as well so callbacks are available
# before the bot connects. This helps ensure buttons from previous messages remain clickable.
try:
    bot.add_view(ApplyButtonView())
    bot.add_view(ApplicationDecisionView())
except Exception:
