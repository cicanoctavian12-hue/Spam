...
# Accept both "!" and "?" prefixes so ?autorole works
...
autoroles: Dict[str, str] = load_json_file(AUTOROLES_PATH, {})  # {guild_id: role_id}
...
# ---------------- /embed command (fixed single definition with short description) ----------------
@tree.command(name="embed", description="Create an embed in a channel by replying with the message.")
@app_commands.describe(
    channel="Target channel to send the embed",
    color="Color name (blue, red) or hex code (#ff0000)",
    picture="Optional image URL to include in the embed"
)
async def embed_command(interaction: discord.Interaction, channel: discord.TextChannel, color: str, picture: Optional[str] = None):
    # Permission check for the invoker (allow managers to use)
    ...
    # Send visible prompt in the same channel where the command was invoked
    await interaction.response.send_message("What message you want to be in embed? (The first message you send here will be used)", ephemeral=False)
