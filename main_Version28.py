...
# API limits observed: button label and component label must be <= 45 characters.
MAX_COMPONENT_LABEL = 45

# ---------------- Application Modal (5 questions with short labels <=45 chars) ----------------
class ApplicationModal(discord.ui.Modal, title="Content Creator Application"):
    ...
    # Labels must be <= 45 chars to satisfy Discord API
    platform_play = discord.ui.TextInput(
        label="1) Which platform do you play on?",
        style=discord.TextStyle.short,
        max_length=500,
        required=True
    )
    platform_post = discord.ui.TextInput(
        label="2) Which platform do you post videos on?",
        style=discord.TextStyle.short,
        max_length=500,
        required=True
    )
    content_type = discord.ui.TextInput(
        label="3) What content do you create?",
    ...
    why_apply = discord.ui.TextInput(
        label="5) Why do you want to be a Content Creator?",
        style=discord.TextStyle.paragraph,
        max_length=500,
        required=True
    )
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        answers = {
            "1": self.platform_play.value,
            "2": self.platform_post.value,
            "3": self.content_type.value,
            "4": self.age.value,
            "5": self.why_apply.value
        }

        # Build purple embed for staff channel listing the Q/A pairs
...
class ApplyButtonView(discord.ui.View):
    ...
    # Persistent callback; label here is only used for the persistent view; actual sent button label is set dynamically when sending message.
    @discord.ui.button(label="Apply", style=discord.ButtonStyle.primary, custom_id=APPLY_BUTTON_CUSTOM_ID, emoji=CREATOR_ICON)
...
# ---------------- /cc_apply slash command (button label required, validated <= MAX_COMPONENT_LABEL) ----------------
...
async def cc_apply(interaction: discord.Interaction, color: str, channel: discord.TextChannel, button: str):
    ...
    button_label = (button or "").strip()
    if not button_label:
        await interaction.response.send_message("Button label cannot be empty.", ephemeral=True)
        return
    if len(button_label) > MAX_COMPONENT_LABEL:
        await interaction.response.send_message(f"Button label is too long. Maximum {MAX_COMPONENT_LABEL} characters allowed.", ephemeral=True)
        return
    ...
    # Primary blue button with validated label and persistent custom_id
    try:
        ...
        view = discord.ui.View(timeout=None)
        view.add_item(discord.ui.Button(label=button_label[:MAX_COMPONENT_LABEL], style=discord.ButtonStyle.primary, custom_id=APPLY_BUTTON_CUSTOM_ID))
